...
module.exports = async function(key, ...values) {
	if (!Util.isValidKey(key)){
		throw new Error(`<rache:list:rpushx> ERR invalid key.`)
	}
	if (!Util.isValidListValue(values)) {
		throw new Error(`<rache:list:rpushx> ERR invalid values.`)
	}
	...
	if (!result){
		...
		throw new Error(`<rache:list:rpushx> WRONGTYPE Operation against a key holding the wrong kind of value.`)
	}
