...
module.exports = async function(key, value, ...opts) {
	if (!Util.isValidKey(key)){
		throw new Error(`<rache:string:set> ERR invalid key.`)
	}
	if (!Util.isValidValue(value)) {
		throw new Error(`<rache:string:set> ERR invalid value.`)
	}
	...
	for (let i = 0; i < opts.length; ++i){
		switch(opts[i]) {
			case "nx":
				if (xx) {
					throw new Error(`<rache:string:set> ERR can not set 'xx' and 'nx' together`)
				}
				nx = true
				break
			case "xx":
			...
			case "ex":
				if (px) {
					throw new Error(`<rache:string:set> ERR can not set 'ex' and 'px' together`)
				}
				++i
				ex = parseInt(opts[i])
				if (!ex || !_.isSafeInteger(ex)){
					throw new Error(`<rache:string:set> ERR Invalid 'ex' value`)
				}
				break
			case "px":
				if (ex) {
				...
				if (!px || !_.isSafeInteger(px)){
					throw new Error(`<rache:string:set> ERR Invalid 'px' value`)
				}
				break
		}
	}
