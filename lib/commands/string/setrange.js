...
module.exports = async function(key, offset, value) {
	if (!Util.isValidKey(key)){
		throw new Error(`<rache:string:setrage> error:: invalid key.`)
	}
	if (offset < 0 || !_.isSafeInteger(offset)){
		throw new Error(`<rache:string:setrage> error:: invalid offset.`)
	}
	if (!Util.isValidValue(value)) {
		throw new Error(`<rache:string:setrage> error:: invalid value.`)
	}
