...
module.exports = async function(topkey, key, value) {
	if (!Util.isValidKey(topkey)){
		throw new Error(`<rache:hash:hincrby> error:: invalid top key.`)
	}
	if (!Util.isValidKey(key)){
		throw new Error(`<rache:hash:hincrby> error:: invalid key.`)
	}
	if (!Util.isValidNumberValue(value)) {
		throw new Error(`<rache:hash:hincrby> error:: invalid value.`)
	}
	...
	if (!data[key]){
		...
		if (isNaN(v)){
			throw new Error(`<rache:hash:incrby> error:: ERR value is not an integer or out of range`)
		}
		data[key] = `${v + value}`
	}
