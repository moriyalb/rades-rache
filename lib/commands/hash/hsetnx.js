...
module.exports = async function(topkey, key, value) {
	if (!Util.isValidKey(topkey)){
		throw new Error(`<rache:hash:hsetnx> ERR invalid top key.`)
	}
	if (!Util.isValidKey(key)){
		throw new Error(`<rache:hash:hsetnx> ERR invalid key.`)
	}
	if (!Util.isValidValue(value)) {
		throw new Error(`<rache:hash:hsetnx> ERR invalid value.`)
	}
